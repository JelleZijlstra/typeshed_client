from typing import IO, Any, Dict, List, Tuple, overload

def get_config_var(name: str) -> str | None: ...
@overload
def get_config_vars() -> Dict[str, Any]: ...
@overload
def get_config_vars(arg: str, *args: str) -> List[Any]: ...
def get_scheme_names() -> Tuple[str, ...]: ...
def get_path_names() -> Tuple[str, ...]: ...
def get_path(
    name: str, scheme: str = ..., vars: Dict[str, Any] | None = ..., expand: bool = ...
) -> str: ...
def get_paths(
    scheme: str = ..., vars: Dict[str, Any] | None = ..., expand: bool = ...
) -> Dict[str, str]: ...
def get_python_version() -> str: ...
def get_platform() -> str: ...
def is_python_build(check_home: bool = ...) -> bool: ...
def parse_config_h(
    fp: IO[Any], vars: Dict[str, Any] | None = ...
) -> Dict[str, Any]: ...
def get_config_h_filename() -> str: ...
def get_makefile_filename() -> str: ...
